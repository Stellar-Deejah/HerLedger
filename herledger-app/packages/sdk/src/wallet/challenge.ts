...
export async function signWalletLinkChallenge(
  ...
  try {
    ...
    throw new WalletError(
      ...
      "Failed to sign wallet verification message with Freighter",
      { cause }
    );
  }
  ...
  if (!result.signedMessage) {
    throw new WalletError(WalletErrorCode.UNAVAILABLE, "Freighter returned no signed message");
  }
  if (typeof result.signedMessage !== "string") {
    ...
    throw new WalletError(
      ...
      "Unexpected signed message format returned by Freighter"
    );
  }
